...
async function aiCall(env, model, messages, maxTokens, stream) {
  ...
  } else if (model.startsWith("openai-direct/") || !model.startsWith("openai/") && !model.startsWith("google/") && !model.startsWith("qwen/") && !model.startsWith("deepseek/") && !model.startsWith("meta-llama/") && OPENAI_DIRECT_MODELS.some((m) => model === m) && env.OPENAI_KEY) {
    provider = "openai";
